...
export default function SendRecommendationScreen() {
  ...
  function toggleRecipient(recipient: Recipient) {
    ...
    const exists = selectedRecipients.some((r) => {
      const rid = r.type === 'contact' ? r.data.id : r.data.id;
      return rid === id;
    });
    if (exists) {
      setSelectedRecipients(selectedRecipients.filter((r) => {
        const rid = r.type === 'contact' ? r.data.id : r.data.id;
        return rid !== id;
      }));
    } else {
  ...
  function isSelected(id: string) {
    return selectedRecipients.some((r) => {
      const rid = r.type === 'contact' ? r.data.id : r.data.id;
      return rid === id;
    });
  }
      ...
      <View style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.scrollContent}>
          <View style={styles.moviePreview}>
            {movie.poster && (
              <Image source={{ uri: movie.poster }} style={styles.previewPoster} />
            )}
          ...
          <View style={styles.rateSection}>
            ...
            {rating ? (
              <View style={styles.ratingDisplay}>
                ...
                <TouchableOpacity onPress={() => setRatingModal(true)} style={styles.editRatingBtn}>
          ...
          <TouchableOpacity
            style={[styles.nextBtn, !rating && styles.disabledBtn]}
            onPress={() => rating && setStep('recipients')}
            disabled={!rating}
          >
        ...
        <RatingModal
          visible={ratingModal}
          title="Votre note pour ce film"
          onConfirm={(r) => { setRating(r); setRatingModal(false); }}
  ...
  if (step === 'recipients') {
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          <TouchableOpacity onPress={() => setStep('rate')} style={styles.backBtn}>
            <Ionicons name="arrow-back" size={22} color={COLORS.text} />
          </TouchableOpacity>
          <Text style={styles.headerTitle}>Destinataires</Text>
          <TouchableOpacity
            onPress={() => selectedRecipients.length > 0 && setStep('recap')}
            disabled={selectedRecipients.length === 0}
          >
            <Text style={[styles.nextText, selectedRecipients.length === 0 && styles.disabledText]}>
              Suivant
            </Text>
          </TouchableOpacity>
        </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent}>
        <View style={styles.moviePreview}>
          ...
          <View style={styles.previewInfo}>
            ...
            {rating?.comment ? (
              <Text style={styles.recapComment} numberOfLines={3}>"{rating.comment}"</Text>
            ) : null}
          </View>
        </View>
