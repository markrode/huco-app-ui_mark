...
export default function LibraryScreen() {
  ...
  function handleRemove(entry: LibraryEntry) {
    Alert.alert('Supprimer', `Retirer "${entry.movie.title}" de votre bibliothèque ?`, [
      { text: 'Annuler', style: 'cancel' },
      { text: 'Supprimer', style: 'destructive', onPress: () => dispatch({ type: 'REMOVE_FROM_LIBRARY', movieId: entry.movie.id }) },
    ]);
  }
        ...
        renderItem={({ item }) => (
          <TouchableOpacity style={styles.item} onPress={() => navigation.navigate('FilmDetails', { movie: item.movie })} activeOpacity={0.8}>
            ...
            <View style={styles.info}>
              ...
              {item.userRating.comment ? <Text style={styles.comment} numberOfLines={2}>{item.userRating.comment}</Text> : null}
              <Text style={styles.date}>Ajouté le {new Date(item.addedAt).toLocaleDateString('fr-FR')}</Text>
            </View>
            <View style={styles.itemActions}>
              <TouchableOpacity style={styles.editBtn} onPress={() => setEditingEntry(item)}><Ionicons name="pencil-outline" size={18} color={COLORS.textMuted} /></TouchableOpacity>
              <TouchableOpacity style={styles.deleteBtn} onPress={() => handleRemove(item)}><Ionicons name="trash-outline" size={18} color={COLORS.primary} /></TouchableOpacity>
            </View>
          </TouchableOpacity>
        )}
        ListEmptyComponent={
          <View style={styles.empty}>
            ...
            <Text style={styles.emptyText}>Ajoutez des films que vous avez vus depuis la recherche ou les détails d'un film.</Text>
            <TouchableOpacity style={styles.searchBtn} onPress={() => navigation.navigate('Search')}><Text style={styles.searchBtnText}>Rechercher un film</Text></TouchableOpacity>
          </View>
        }
        contentContainerStyle={state.library.length === 0 ? styles.emptyContainer : { paddingBottom: SPACING.xl }}
        showsVerticalScrollIndicator={false}
      />
      <RatingModal visible={!!editingEntry} title={`Modifier ma note — ${editingEntry?.movie.title}`} onConfirm={handleUpdateRating} onCancel={() => setEditingEntry(null)} initialRating={editingEntry?.userRating} />
    </View>
  );
}
